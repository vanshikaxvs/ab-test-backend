...
def init_db():
    ...
    c.executescript("""
        CREATE TABLE IF NOT EXISTS visits (
            ...
            session_dur INTEGER DEFAULT 0,       -- seconds
            bounce      INTEGER DEFAULT 0,       -- 1 = bounced
            converted   INTEGER DEFAULT 0,       -- 1 = signed up
            color_pref  TEXT DEFAULT NULL,       -- favourite colour picked
...
def compute_stats(a_visits, a_conv, b_visits, b_conv):
    """
    Returns a dict with all statistical results.
    Uses two-proportion z-test at 95% confidence (alpha=0.05).
    Effect size = Cohen's h.
    Power analysis using statsmodels or manual formula.
    """
    result = {
    ...
    # 95% Confidence Interval for (p_b - p_a)
    diff = p_b - p_a
    ...
    if se > 0:
        power_z = (abs(diff) - z_alpha * se) / (se)  # simplified
        power   = stats.norm.cdf(power_z)
        result["power"] = round(float(power) * 100, 1)

    # Recommended sample per variant (Cohen's h method)
    if abs(h) > 0:
    ...
    if a_visits + b_visits < 100:
        result["verdict"] = "More data needed — run the test longer"
    elif result["significant"] and p_b > p_a:
        result["verdict"] = "Ship Variant B — statistically significant improvement"
    elif result["significant"] and p_b <= p_a:
        result["verdict"] = "Keep Variant A — Variant B underperforms"
    elif result["power"] < 80:
        result["verdict"] = "Test inconclusive — insufficient statistical power"
    else:
        result["verdict"] = "No significant difference — consider iterating design"
...
def log_visit():
    """
    Called when a user lands on your landing page.
    Assigns them to A or B (50/50), stores in DB.
    Returns: { visit_id, variant }
    """
...
def log_conversion():
    """
    Called when a user signs up / clicks CTA.
    Body: { visit_id, session_duration (seconds), color_pref }
    """
    data    = request.json or {}
...
def log_bounce():
    """Called when user leaves without converting."""
    data = request.json or {}
    vid  = data.get("visit_id")
    dur  = data.get("session_duration", 0)
    if vid:
...
def log_color_vote():
    """Stand-alone colour preference vote (without conversion)."""
    data    = request.json or {}
...
def get_results():
    """
    Returns full stats for the dashboard.
    """
    conn = get_db()
...
def seed_demo():
    """
    Populates the DB with realistic synthetic data so you can see the dashboard immediately.
    ...
    base_ts = int(time.time()) - 7 * 24 * 3600   # 1 week ago

    for i in range(500):
        variant = "A" if i < 250 else "B"
        vid = str(uuid.uuid4())
        ts  = base_ts + random.randint(0, 7 * 24 * 3600)
        # Variant B has higher conversion rate (12% vs 8%)
...
@app.route("/")
def dashboard():
    """Serve a basic landing page pointing to the React dashboard."""
    return """
